...
class UserTest < ActiveSupport::TestCase
  ...
  test 'should set active to false by default' do
    user = User.new(
      name: 'someone', email: 'someone@test.com', password: 'password'
    )
  ...
  test 'membership_active? should return true if current_period_end is in
  future' do
  ...
  test 'membership_active? should return false if current_period_end is in
  past' do
    Timecop.travel(Date.current)
    user = users(:user)
    user.current_period_end = Date.current - 1
  ...
  test 'admin? should return admin property' do
    user = users(:user)
  ...
  test '#end_date sets current_period_end to event period_end with webhook
  event' do
  ...
  test '#end_date sets current_period_end to event current_period_end with
  subscription' do
    user = users(:user)
    subscribe(user)
    assert_in_delta 1.month.from_now, user.current_period_end, 200
  ...
  test 'wont allow duplicate subscription' do
    user = users(:user)
    subscribe(user)
  ...
  test '#subcribe should re-activate the subscription if
  within current_end_period' do
    user = users(:user)
    subscribe(user)
  ...
  test '#subscribe should attempt to subscribe if unable to
  re-activate' do
    # TODO: implement
    # this is where the question of, does the subscription
    # object still exist on the Stripe::Customer after the
    # subscription has lapsed (passed the end date)? if so,
    # then we have to re-make the subscription object and
    # apply it to the user, otherwise, we can re-activate
  end
  ...
  test 'membership should still be active if within current_end_date
  after #cancel' do
    user = users(:user)
    subscribe(user)

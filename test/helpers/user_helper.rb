module UserHelper
  def set_stripe_user(user)
    ...
    # once the test starts, StripeMock doesn't recognize it because it hasn't generated
    # it yet... But then if I don't put anything in, I can't test for validity.
    user.stripe_customer_id = Stripe::Customer.create.id if user.stripe_customer_id == 'delete'
    user.stripe_customer_id ||= Stripe::Customer.create.id
  end

  def subscribe(user, token = nil)
    set_stripe_user(user)
